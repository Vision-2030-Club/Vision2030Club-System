...
export function TvBoard({
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-brand-950 text-white">
      <header className="flex items-center justify-between px-8 py-4 text-brand-200">
        <span className="text-xl font-semibold">{title}</span>
        <span className="ltr-nums text-2xl font-semibold tabular-nums">{clock}</span>
      </header>
      ...
      <main className="flex min-h-0 flex-1 flex-col px-8">
        <section className="flex min-h-0 flex-[3] flex-col items-center justify-center text-center">
          {calling.length ? (
            <>
              <div className="mb-4 text-xl font-medium uppercase tracking-[0.3em] text-brand-300">{labels.nowCalling}</div>
              <ul className="space-y-4">
                {calling.map((b) => (
                  <li key={b.booking_id} className="motion-safe:animate-pulse">
                    ...
                    <div className="mt-3 text-[clamp(1.25rem,3vw,2.5rem)] text-brand-100">
            ...
            <div className="text-[clamp(1.5rem,4vw,3rem)] text-brand-300">{labels.quiet}</div>
          )}
        </section>

        <section className="min-h-0 flex-[2] border-t border-brand-800 pt-4">
          <div className="mb-2 text-sm font-medium uppercase tracking-[0.25em] text-brand-300">{labels.upNext}</div>
      ...
      <footer className="overflow-hidden border-t border-brand-800 bg-brand-900 py-3">
        <div className="tv-ticker flex w-max gap-12 whitespace-nowrap px-8 text-lg text-brand-100">
          {[...board.ticker, ...board.ticker].map((item, i) => (
            <span key={`${item.room_name}-${i}`} className="flex items-baseline gap-2">
              ...
              <span className="text-brand-300">{companyName(item)}</span>
              <span>
